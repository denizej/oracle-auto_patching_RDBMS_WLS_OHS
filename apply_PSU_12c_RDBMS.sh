...
function usage()
{
  echo
  echo "Usage:"
  echo "$THIS_SCRIPT [-component=COMP_TYPE] [-version=ORA_VER] [-PSU=PSU_VER] [-SID=ORACLE_SID] {-stageDir=BASE_DIR} {-doDatapatch} {-noGrid} {-pdb} {-noPrompt} {-debug} {-dryRun} {-help}"
  echo
  echo "Where:"
  echo "  -component|-c    - Required. Specify the Oracle component stop and apply patches to (currently only RDBMS is valid)"
  echo "  -version|-v      - Required. Specify the Oracle 12c database version (12.1.0.2 or 12.2.0.1)"
  echo "  -PSU|-psu        - Required. Specify the Oracle PSU date to apply to the Oracle Home (format must be YYMMDD and currently only 180717 and 181016 are valid)"
  echo "  -SID|-sid        - Required. Specify the Oracle SID to lookup from the ORATAB file"
  echo "  -stageDir|-s     - Optional. Staging directory of where to find the patches to apply (default is $BASE_DIR/psu/${COMP_TYPE}_${ORA_VER}_${PSU_VER})"
  echo "  -doDatapatch|-dp - Optional. Setting this option this will run OPatch/datapatch after attaching the new Oracle Home (default is not to run datapatch)"
